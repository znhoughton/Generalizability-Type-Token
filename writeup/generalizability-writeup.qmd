...
A crucial element of language learning is the ability to use that language beyond the contexts in which it is encountered. This is not a trivial task since word-frequencies follow a Zipfian distribution [@zipf1949principleleasteffort], with the most frequent word being twice as frequent as the second most frequent word, which in turn is twice as frequent as the third most frequent word (so on and so forth). This means that the majority of words lie in the tail end of the distribution, with humans having encountered them infrequently. As a result, in order to use language outside of the contexts in which it is encountered, language learning mechanisms must enable us to infer meaning beyond the exact contexts.

Put another way, humans must be able to generalize beyond the specific contexts in which language is heard in. For example, a child having heard the word *kitty* in limited contexts must at some point learn that this word applies to a kitten that the child has never seen before, but not to a puppy which may share many features with the kitten.

In order to proficiently use language, humans must generalize language in many different ways. For example, humans must learn whether a certain morphological operation should be applied to a new word, such as whether to extend the regular past tense rule to *dive* to produce *dove* or whether to analogize *dive* to *dove* (formed in analogy to *drive-drove*). Additionally, humans must also learn whether to extend a word to a novel meaning. For example, to continue with the previous example, a child must learn that the word *kitty* still refers to a kitten that has only 3 legs or only 1 eye. On the other hand, the word *kitty* should not apply to a lion (conceding the exception, of course, that one can in principle creatively overextend the word *kitty* to a lion in order to convey a specific meaning).

There is good evidence that children are able to effectively generalize. For example, @xuWordLearningBayesian2007 demonstrated that children are able to generalize a word's meaning beyond the literal context it is heard in. Specifically, they examined how children come to learn that a novel word refers to a hypernym such as dalmatian as opposed to the respective hyponym (dog). They found that when encountering only one example of the novel word with a dalmatian, children assume that the word refers to the hyponym. However, when they encounter the word in three separate instances, each of which occurring with an image of a dalmatian, the children instead assume that the word refers to the hypernym (dalmatian). This, @xuWordLearningBayesian2007 argued, is the rational conclusion because it is unlikely that the word would refer to the hyponym, dog, and occur three separate times with a dalmatian (without occurring with any other dogs).

Additionally, @smith2008infantsrapidlylearn demonstrated that humans can generalize a word's meaning across several different contexts, even when each individual context is ambiguous as to the meaning of the word. In order to test this, they showed infants two pictures at a time paired with a single word. While the word was ambiguous within a single trial (it could refer to either image), it was unambiguous across trials. That is, one of the two images that were presented always co-occurred with its corresponding label. They found that despite the ambiguity within trials, during the test phase infants spent a longer time looking at the image that the word referred to, despite the ambiguity of the individual trials. This demonstrates that humans are able to generalize beyond the specific context by taking advantage of statistical regularities across different contexts.

While there is a great deal of evidence demonstrating humans' abilities to generalize, it remains unclear what factors drive generalization. For example, while some have argued that generalizations are driven by accessibility [e.g., @harmonPuttingOldTools2017], others have argued that generalizations are instead driven by rational inference [e.g., @xuWordLearningBayesian2007]. The present paper explores the mechanisms driving generalizations in humans by exploring a specific type of generalization, semantic extension, using an artificial language learning task.

### Linguistic Productivity

<!--# Expand on this section -->

One way in which humans show an incredible ability to generalize language to novel contexts is through the creativity in which humans can easily extend learned forms to new contexts. Perhaps the most famous example of this linguistic productivity comes from @berkoChildsLearningEnglish1958. In her seminal paper, she examined whether children could use the correct morphological operations to novel words. She found that children could apply the correct pluralization to novel words that they have never heard before, such as pluralizing "wug" to "wugs". Her study was extremely influential because it demonstrated that children are not simply memorizing language, but learning to productively extend their language to novel contexts.

Linguistic productivity provides a window into how language is learned. This is especially clear in cases where there are multiple competing productive forms that a learner can choose from. For example, while historically the past tense of the word *dive* is *dived*, contemporarily the alternative form *dove* is acceptable and has infact overtaken *dived* as the more frequent form (while *dived* also remains perfectly acceptable). Similarly, in Russian velar stops undergo palatalization without exceptions in the Russian lexicon before certain verb stem extensions [@kapatsinskiVelarPalatalizationRussian2010]. However, recently this palatalization has failed in some of these contexts for some loanwords into Russian. @kapatsinskiVelarPalatalizationRussian2010 demonstrated that palatalization is fully productive before -*ek* and -*ok* but only half-productive (i.e., palatalization occurs \~50% of the time) for *-i* and -*ik*. Using a combination of corpus methods and an artificial language learning study, @kapatsinskiVelarPalatalizationRussian2010 argued that the difference in productivity of the palatalization was due to the type-frequency of palatalization before the specific contexts: *-ik* and *-ok* do not frequently occur with different velars while *-i* and -*ik* do. In other words, learners are sensitive to how often palatalization takes place in each context in order to infer the productivity of the morphological process.

In addition to type frequency, token frequency also seems to play a role in linguistic productivity as well [@jarosz2025typetokenfrequency].

The effects of type and token frequency on linguistic productivity suggest that linguistic productivity may be driven by the level of accessibility of a form in a given context. Accessibility refers to how easy a given representation can be accessed. Frequency effects on accessibility are extremely well-documented. For example, frequent words show faster reading times [@rayner2009eyemovementsattention; @drieghe2005eyemovementsword], higher skip-rates in reading [@drieghe2005eyemovementsword], and phonetic reduction in spoken speech [@bybee1999usagebasedphonology]. While these metrics are typically estimates of token frequency (e.g., word frequency), type frequency has similarly been demonstrated to affect accessibility. For example, it is easier for people to recognize words if they have a large morphological family size [i.e., if the stem occurs with many different words, @dejong2000]. Similarly, phrases with high predictability (i.e., low type frequency) also seems to be stored holistically regardless of the frequency of the phrase [@houghton2025multiwordrepresentationsminds].

However, it is unclear if the mechanisms driving linguistic productivity are the same mechanisms that drive generalizations more broadly in humans. For example, there is great debate about whether the factors that drive linguistic productivity similarly affect whether humans extend learned forms to express novel meanings [e.g., @perfors2014; @xuWordLearningBayesian2007]. These arguments are driven by studies that have demonstrated that a high token frequency without a high type frequency leads comprehenders to be less likely to accept a form with a novel meaning [@xuWordLearningBayesian2007; @theakstonRoleEntrenchmentChildren2004].

### Semantic Extension

<!--# talk about Bybee arguing that extension is different than generalization -->

In addition to linguistic productivity, humans also sometimes generalize a word's meaning beyond the original meaning of the word [i.e. semantic extension, @harmonPuttingOldTools2017; @naigles1995overextensionscomprehensionproduction]. For example, children will sometimes use the word *kitty* to mean all four-legged animals [@naigles1995overextensionscomprehensionproduction]. Semantic extension is further evidenced by trends in language change, such as the trend for frequent forms to be grammaticalized [@bybee2017mechanismschangegrammaticization]. For example, historically as *going to* increased in frequency it became semantically extended beyond motion to encompass the future tense in English as well [@bybee2017mechanismschangegrammaticization]. This is a natural consequence of semantic extension because as a frequent form becomes extended to more new contexts, its meaning becomes more and more bleached.

Semantic extension plays a crucial role in language production because it enables us to use familiar forms to describe novel meanings [@harmonPuttingOldTools2017]. Despite its importance, it remains unclear what factors drive semantic extension. Mainly, it is unclear whether semantic extension is driven by accessibility or by rational learning.

Accessibility, as the term suggests, refers to how easy it is for a learner to access a given representation. Accessibility is driven by many different factors, such as the frequency of the representation or the current attention of the learner.

Accessibility plays a large role in language production and processing.

For example, there is an abundance of evidence that binomial ordering preferences (e.g., *bread and butter* vs *butter and bread*) are driven at least in part by accessibility [@morganAbstractKnowledgeDirect2016; @goldberg2021] and that the changing of these ordering preferences over time (e.g., the reversal of the preference of *dad and mom to mom and dad*) reflects changes in the accessibility of that form. Further evidence of the role of accessibility in language processing comes from Russell Tomlin's seminal *fish film* [@tomlin1995focalattentionvoice]. In his study, he presented participants with a short video of one fish eating another fish, with one of the fishes having an arrow over its image. He found that if the image with an arrow over its head was the fish being eaten, participants would describe the image using the passive form. If the image was instead, the fish doing the action of the eating, participants would instead use the active form. The study demonstrates that accessibility plays a crucial role in the way we produce language.

Accessibility provides a fairly intuitive account for semantic extension: when encountering a new context, selecting the most accessible word that can fit the new context seems like a reasonable approach. Indeed, @harmonPuttingOldTools2017 demonstrated that accessibility may play a role in semantic extension using an artificial language learning experiment. In their experiment, participants were presented with an image of a creature along with the name of that creature. In their artificial language, the stem for each creature was followed by one of four suffixes, which varied in how frequently they occurred with each stem. In the test case, participants were asked to produce the correct suffix to express a novel meaning. Crucially, two of the suffixes were compatible with the novel meaning, but varied in the frequency that they occurred in training. @harmonPuttingOldTools2017 found that in production, participants were more likely to use the frequent suffix. Interestingly, they also found that in a two-alternative forced choice task, participants were equally likely to select each suffix (i.e., there was no effect of frequency). They argued that this was because in a two-alternative forced choice task, both forms were made equally accessible, thus minimizing the difference in accessibility between the two suffixes.

While accessibility seems to be a plausible explanation, it may instead be the case that semantic extension is driven by rational learning – learning the extent to which a specific linguistic cue is informative about the meaning. Indeed, this is the foundation that underlies error-driven learning theories. Rational learning has been demonstrated to play an important role in language learning more broadly. For example, @ramscarChildrenValueInformativity2013 demonstrated that in word-learning, children pay attention not only to simple co-occurrence rates, but also to how *informative* the word is of the meaning. Using an artificial language learning task, @ramscarChildrenValueInformativity2013 examined how children and adults learn the meanings of novel words. In their study, participants saw two objects together (A, B, or C) along with an ambiguous label (either *wug*, *pid*, or *dax*). Additionally, object C occurred in every trial. Crucially, *dax* always occurred with A and *pid* always occurred with C, however *wug* did not consistently occur with either. across different trials one of the objects was always the same. @ramscarChildrenValueInformativity2013 then asked participants which object corresponded to the label *wug*. They found that children selected either A or C. This is because they learned to ignore object C since it was uninformative (it occurred in every trial and thus was uninformative).

Beyond word-learning, rational learning also plays a role in the learning of phonetic categories. For example, @olejarczukDistributionalLearningErrordriven2018 examined how humans learn new phonetic categories. They found that learners' perceptual categories are disproportionately affected by rare members of a phonetic category. Their findings suggest that learners actively pay attention not just to co-occurrence rates, but also how informative each member is of the phonetic category and update their beliefs in proportion to how surprising their membership is, providing further evidence for rational learning.

While there is evidence that rational learning plays a large role in language learning [e.g., @viviani2026godoesinput; @jarosz2025typetokenfrequency; @olejarczukDistributionalLearningErrordriven2018; @ramscarChildrenValueInformativity2013], it is unclear whether it may play a role in semantic extension. For example, while @harmonPuttingOldTools2017 argued that their findings demonstrate the role of accessibility in semantic extension, their results are also compatible with a rational learning account since the frequent suffix also occurred with more unique creatures than the infrequent suffix (i.e., it had high type frequency). This may have led participants to produce the more frequent suffix in a novel context because they believe that it is the most informative option. While the results of the two-alternative forced choice task seem to contradict this possibility, participants may accept both possibilities when explicitly asked because they are both technically compatible with the new meaning.[^1]

[^1]: Indeed, @qiu2023taskvariationscalar and @houghtonTaskdependentConsequencesDisfluency2024 have both found that the tasks that participants are engaged in can impact the way that they process language. For example, when participants are asked whether it is true that all the images on a card are cats upon hearing the sentence "some animals are cats", they will answer yes despite the implicature that not all the animals on the card are cats.

Finally, it is possible that both accessibility and rational learning may play a role in semantic extension. This seems plausible since humans employ both Hebbian [@hebb1949firststageperception] and error-driven learning mechanisms [@ashbyNeurobiologicalTheoryAutomaticity2007, @kapatsinskiChangingMindsChanging2018]. In other words, humans are sensitive to not only frequency, but also informativity. It is possible that both of these play a crucial role in semantic extension. This is further supported by evidence that both accessibility and rational learning play a role in linguistic productivity [@jarosz2025typetokenfrequency]. For example, in an artificial language learning study, @jarosz2025typetokenfrequency tested whether type frequency or token frequency drive humans' ability to produce competing plural suffixes with novel stems. They found that both type frequency and token frequency contribute to linguistic productivity, suggesting that both accessibility and rational learning play a role. It seems likely that this may be the case for semantic extension as well.

### Present Study

Thus in the present study, we expand upon the findings in @harmonPuttingOldTools2017 by examining whether accessibility or rational learning drives semantic extension. In order to do so, we manipulate independently the token frequency of the target suffix (the number of times the suffix occurs, regardless of the stem that it occurs with) and the type frequency of the target suffix (the number of different stems the suffix occurs in). We argue that token frequency acts as a good proxy for accessibility given the abundance of studies demonstrating that frequency increases accessibility and that type frequency acts as a good proxy for rational learning because a rational learner should conclude that a suffix that occurs in more contexts is more likely to be used in a novel context.
...
[^2]: Additionally, a demo of the experiment can be found at the following link: <https://run.pavlovia.org/znhoughton/generalizability_demo>.
...
We explore the results for each task in depth in the next sections.[^3]

[^3]: All data and code for the analyses can be found here: <https://github.com/znhoughton/Generalizability-Type-Token>.
...
Our results suggest that in production, a high type and high token frequency together lead to a preference for the frequent suffix, and this preference is even stronger if the stem is novel. On the other hand, when there is a high type frequency but not a high token frequency, there is only a preference for the frequent form when the stem and meaning are both novel. Finally, for token frequency there is only a preference for the frequent form when the meaning is original. Overall, these results suggest that type frequency and token frequency both play a role in semantic extension, with a high type frequency encouraging semantic extension while a high token frequency encourages using the suffix to communicate the original meaning. As was the case in @harmonPuttingOldTools2017, these effects are mitigated when both forms are brought into memory.
...
Finally, our results also demonstrate that the logistic perceptron is able to capture the human data. Interestingly, the model predicts the human data best when configural cues and an increased salience for semantic cues relative to phonological cues are included. These modeling results provide an additional piece of evidence to the rapidly growing literature suggesting that human learning involves storing holistic representations of complex units [e.g. multi-morphemic or multi-word representations, @houghton2025multiwordrepresentationsminds; @morganAbstractKnowledgeDirect2016; @morganModelingIdiosyncraticPreferences2015; @morgan2024productiveknowledgeitemspecific; @stembergerAreInflectedForms2004; @stembergerFrequencyLexicalStorage1986]. The support for the holistic storage of complex forms largely come from findings in language processing or language change, such as evidence that the component parts of frequent words are harder to recognize [@kapatsinski2010frequencyuseleads], that high frequency words are more susceptible to phonetic reduction [e.g., *don't* is more reduced in *I don't know* than in *I don't go*, @bybee1999usagebasedphonology], or that high-frequency binomials become more regularized in their ordering preferences [@morganFrequencydependentRegularizationIterated2016a; @houghton2025boltsnutslanguage]. However, comparatively few studies outside of the present one have demonstrated computationally that modeling holistic storage leads to better prediction of human learning.

Overall, the results of this study demonstrate that both accessibility and rational learning play a role in semantic extension, evidenced by the joint effect of type and token frequency on semantic extension. This is inline with previous results that demonstrate that both type and token frequency play a role in linguistic productivity [@jarosz2025typetokenfrequency].

An important caveat to these results is that rational learning and accessibility, although presented as two disjoint mechanisms, may not be unconnected. It is entirely possible that rational learning *leads* to increased accessibility. For example, it may still be the case that semantic extension is primarily driven by accessibility. However, a suffix that occurs with many different suffixes may itself be more accessible with a novel meaning (especially with a novel form) simply because the representation has an increased number of connections to other representations (in this case, the other stems). This seems to be supported by our modeling results, where accessibility and rational learning are not implemented as two separate mechanisms. In a connectionist model like the one we used, while learning is driven by prediction-error, there is no explicit implementation of accessibility. Instead, accessibility is simply represented through the strength of the connection between forms and meanings, which is learned through prediction error. It seems plausible that human learning may be similar to this.

Further, in the same framing, semantic extension and linguistic productivity may reflect the same underlying mechanism. Both of these phenomena can be framed as the learner selecting the form that is most accessible given the current context. Whether the learner is trying to determine which form to select to express a novel meaning or whether the learner is deciding whether which plural form should be used with a novel noun, in either case the learner may simply be selecting the most accessible representation.
