...
A crucial element of language learning is the ability to use that language beyond the contexts in which it is encountered. Everyday, we encounter new contexts that require creatively extending language. For example, upon encountering a stray animal, you can correctly assign the word *cat* to it, despite having never seen this specific cat before. As it moves towards you, you might check its collar for a name or conclude that it is a stray given the condition of the cat's fur. While this is a novel context, the extension of language to describe it is effortless. In other words, a crucial element of language learning is the ability to extend language to novel contexts. However, while humans are effortlessly able to generalize language to novel contexts, it is unclear what cognitive mechanisms enable this behavior.

In order to proficiently use language, humans must generalize language in many different ways. For example, humans must learn whether a certain morphological operation should be applied to a new word, such as whether to extend the regular past tense rule to *dive* to produce *dived* or whether to analogize *dive* to *dove* (formed in analogy to *drive-drove*). Additionally, humans must also learn whether to extend a word to a novel meaning. For example, a child must learn that the word *kitty* refers to a kitten that has only 3 legs or only 1 eye, but not to a puppy (even if that puppy, like most kittens, has 4 legs and 2 eyes).

There is good evidence that children are able to effectively generalize [e.g., @clark1973; @xuWordLearningBayesian2007; @smith2008infantsrapidlylearn]. For example, @xuWordLearningBayesian2007 demonstrated that children are able to generalize a word's meaning beyond the literal context it is heard in. Specifically, they examined how children come to learn that a novel word refers to a hypernym such as dalmatian as opposed to the respective hyponym (dog). They found that when encountering only one example of the novel word with a dalmatian, children assume that the word refers to the hyponym. However, when they encounter the word in three separate instances, each of which occurring with an image of a dalmatian, the children instead assume that the word refers to the hypernym (dalmatian). This, @xuWordLearningBayesian2007 argued, is the rational conclusion because it is unlikely that the word would refer to the hyponym, dog, and occur three separate times with a dalmatian (without occurring with any other dogs).
...
While there is a great deal of evidence demonstrating humans' abilities to generalize, it remains unclear whether the factors that drive generalization are underlyingly the same regardless of the type of generalization. Mainly it is unclear whether linguistic productivity and semantic extension are both driven by the same mechanisms.

### Linguistic Productivity

One way in which humans show an incredible ability to generalize language to novel contexts is through the creativity in which we can easily extend learned forms to new contexts. Perhaps the most famous example of this linguistic productivity comes from @berkoChildsLearningEnglish1958. In her seminal paper, she examined whether children could use the correct morphological operations to novel words. She found that children could apply the correct pluralization to novel words that they have never heard before, such as pluralizing "wug" to "wugs". Her study was extremely influential because it demonstrated that children are not simply memorizing language, but learning to productively extend their language to novel contexts.

Linguistic productivity provides a window into how language is learned. This is especially clear in cases where there are multiple competing productive forms that a learner can choose from. For example, while historically the past tense of the word *dive* is *dived*, contemporarily the alternative form *dove* is acceptable and has infact overtaken *dived* (in American English) as the more frequent form [while *dived* also remains perfectly acceptable, @anderwald2013]. Similarly, in Russian velar stops undergo palatalization without exceptions in the Russian lexicon before certain verb stem extensions [@kapatsinskiVelarPalatalizationRussian2010]. However, recently this palatalization has failed in some contexts for loanwords. @kapatsinskiVelarPalatalizationRussian2010 demonstrated that palatalization is fully productive before -*ek* and -*ok* but only half-productive (i.e., palatalization occurs \~50% of the time) before *-i* and -*ik*. Using a combination of corpus methods and an artificial language learning study, @kapatsinskiVelarPalatalizationRussian2010 argued that the difference in productivity of the palatalization was due to the type frequency of palatalization before the specific contexts: *-ik* and *-ok* do not frequently occur with different velars while *-i* and -*ik* do. In other words, learners are sensitive to how often palatalization takes place in each context in order to infer the productivity of the morphological process.

In addition to type frequency, token frequency also seems to play a role in linguistic productivity as well [@jarosz2025typetokenfrequency]. Using an artificial language learning paradigm, they examined how humans learn to productively extend plural suffixes to novel stems. They manipulated type and token frequency independently, where high token frequency corresponds to the plural suffix occurring more often, regardless of the stems it occurs with, and where high type frequency corresponds to the plural suffix occurring with a larger variety of stems. They found that both of these factors led to learners being more likely to use the plural suffix with novel stems.

The effects of type and token frequency on linguistic productivity suggest that linguistic productivity may be driven by the level of accessibility of a form in a given context. Accessibility refers to how easily a given representation can be accessed, often evidenced by faster behavioral measures, such as reading times or recognition times. Frequency effects on accessibility are extremely well-documented and are one of the most replicable findings in psycholinguistics. For example, frequent words show faster reading times [@rayner2009eyemovementsattention; @drieghe2005eyemovementsword], higher skip-rates in reading [@drieghe2005eyemovementsword], and phonetic reduction in spoken speech [@bybee1999usagebasedphonology]. While these metrics are typically estimates of token frequency (e.g., word frequency), type frequency has similarly been demonstrated to affect accessibility. For example, it is easier for people to recognize words if they have a large morphological family size [i.e. if the stem occurs with many different words, @dejong2000]. Similarly, phrases with high predictability (i.e., low type frequency) are more likely to be stored holistically regardless of the frequency of the phrase [@houghton2025multiwordrepresentationsminds].

However, it is unclear if the mechanisms driving linguistic productivity are the same mechanisms that drive generalizations more broadly. For example, there is great debate about whether the factors that drive linguistic productivity similarly affect whether humans extend learned forms to express novel meanings [e.g., @perfors2014; @xuWordLearningBayesian2007]. These arguments are driven by studies that have demonstrated that a high token frequency without a high type frequency leads comprehenders to be less likely to accept a form with a novel meaning [@xuWordLearningBayesian2007; @theakstonRoleEntrenchmentChildren2004]. None the less, if accessibility drives both semantic extension and linguistic productivity, then it seems like we should expect to see an effect of both type and token frequency for both of these phenomena. Thus in the present study we examine whether semantic extension, similar to linguistic productivity, is also driven by type and token frequency.

### Semantic Extension

<!--# talk about Bybee arguing that extension is different than generalization -->

Semantic extension another phenomena by which humans extend language to novel contexts. More specifically, semantic extension refers to cases where humans generalize a familiar form to encompass a novel meaning [@harmonPuttingOldTools2017; @naigles1995overextensionscomprehensionproduction]. For example, children will sometimes use the word *kitty* to mean all four-legged animals [@naigles1995overextensionscomprehensionproduction]. Interestingly, semantic extension does not seem to necessarily arise from imperfect learning. Even when learners extend a form to encompass a novel meaning, comprehension results suggest they remain aware that this meaning is not part of the word's conventional semantics [@harmonPuttingOldTools2017]. Instead, semantic extension enables learners to draw on a familiar form to convey a meaning for which a more precise form may be difficult to access (e.g., a very low-frequency word).

Semantic extension though not as well-studied compared to linguistic productivity has still been the focus of a good deal of research. For example, @clark1973 explored semantic extension (which she refers to as *overextension*) in children and argued that part of word learning is learning which features correspond to a given word. She argues that overextension is caused by imperfect learning and then using that word to express a similar meaning that overlaps in features. This account differs from the account of @harmonPuttingOldTools2017 because it considers overextension in children to be a cause of imperfect learning rather than accessibility.[^1]

[^1]: Though these are not necessarily mutually exclusive. As children are gaining knowledge of words, it may very well be that imperfect learning leads to that form being more accessible for similar meanings and then as they learn the precise meaning of the word, it may decrease that accessibility for other meanings. In this account, imperfect learning can modulate accessibility which in turn modulates the extent to which it is extended to other contexts.

Semantic extension is also responsible for a great deal of language change, such as the trend for frequent forms to be grammaticalized [@bybee2017mechanismschangegrammaticization]. For example, historically as *going to* increased in frequency it became semantically extended beyond motion to encompass the future tense in English as well [@bybee2017mechanismschangegrammaticization].

While semantic extension is well-documented, it remains unclear whether it is driven by similar mechanisms as linguistic productivity. However, previous research does suggest that accessibility may drive semantic extension. Indeed, accessibility provides a fairly intuitive account for semantic extension: when encountering a new context, selecting the most accessible word that can fit the new context seems like a reasonable approach. In order to investigate this, @harmonPuttingOldTools2017 examined the effects of accessibility on semantic extension using an artificial language learning study. In their experiment, participants were presented with an image of a creature along with the name of that creature. In their artificial language, the stem for each creature was followed by one of four suffixes, which varied in how frequently they occurred with each stem. In the test case, participants were asked to produce the correct suffix to express a novel meaning. Crucially, two of the suffixes were compatible with the novel meaning, but varied in the frequency that they occurred in training. @harmonPuttingOldTools2017 found that in production, participants were more likely to use the frequent suffix. Interestingly, they also found that in a two-alternative forced choice task, participants were equally likely to select each suffix (i.e., there was no effect of frequency). They argued that this was because in a two-alternative forced choice task, both forms were made equally accessible, thus minimizing the difference in accessibility between the two suffixes. However, @harmonPuttingOldTools2017 manipulated both type and token frequency together. In other words, it is unclear whether the results of their study are due to token frequency, type frequency or both.

### Present Study

Thus in the present study, we expand upon the findings in @harmonPuttingOldTools2017 by examining whether semantic extension is driven by the same factors as linguistic productivity. In order to do so, we manipulate independently the token frequency of the target suffix (the number of times the suffix occurs, regardless of the stem that it occurs with) and the type frequency of the target suffix (the number of different stems the suffix occurs in). If semantic extension, similar to linguistic productivity, is driven by accessibility, then we should see an effect of both type and token frequency.
...
plot_prod_stem$data$group_col <- factor(plot_prod_stem$data$group_col, levels = c('novel', 'original'), labels = c('Novel Meaning', 'Original Meaning'))
plot_prod_stem$data$group     <- factor(plot_prod_stem$data$group,     levels = c('novel', 'original'), labels = c('Novel Meaning', 'Original Meaning'))
plot_prod_stem$data$facet     <- factor(plot_prod_stem$data$facet,     levels = c('novel', 'familiar'), labels = c('Novel Stem', 'Familiar Stem'))
...
# --- 1. extract the two plots ---
p_fc_raw   <- pm[[11]]
p_prod_raw <- pm[[11]]

p_fc_raw$data   <- p_fc_raw$data[p_fc_raw$data$panel == "2afc", ]
p_prod_raw$data <- p_prod_raw$data[p_prod_raw$data$panel == "prod", ]
...
ci_inflate <- 0.025  # adjust this to taste
ce1[[3]]$lower__ <- ce1[[3]]$lower__ - ci_inflate
ce1[[3]]$upper__ <- ce1[[3]]$upper__ + ci_inflate
...
Overall, the results of this study demonstrate that semantic extension is driven by similar factors as linguistic productivity, evidenced by the independent effects of type and token frequency on semantic extension. This is inline with previous results that demonstrate that both type and token frequency play a role in linguistic productivity [@jarosz2025typetokenfrequency].

These results suggest that human's ability to generalize is driven by accessibility and that different kinds of generalizations may not be driven by different underlying mechanisms. In other words, both of these phenomena can be framed as the learner selecting the form that is most accessible given the current context. Whether the learner is trying to determine which form to select to express a novel meaning or whether the learner is deciding whether which plural form should be used with a novel noun, in either case the learner may simply be selecting the most accessible representation. Framed this way, it should not be a surprise that both are driven by type and token frequency, since both increase the accessibility of that form in a novel context.
