...
1.  Hook (how do we extend our language to novel contexts?
2.  Semantic Extension
    1.  What is it?
    2.  What drives it?
        1.  Accessibility? Informativity?
3.  Discussion point: Point out that informativity may also be another form of accessibility: low informativity = high predictability, more likely to be stored

## Introduction

As humans, the ability to adapt to novel contexts is essential to our survival as a species. Language is no exception to that: the language we encounter and produce is often novel. For example, word-frequency follows a Zipfian distribution [@zipf1949principleleasteffort], where the most frequent word is twice as frequent as the second most frequent word which is twice as frequent as the third most frequent word, so on and so forth. This means that the majority of words fall in the tail end of the distribution, with humans encountering them extremely rarely. As a result, in order to learn and process language well, we must encode language in a way that enables flexibility.

Put another way, humans must be able to generalize a form or meaning beyond the specific contexts in which it is heard in. For example, a child having heard the word *kitty* in limited contexts must at some point learn that this word applies to a kitten that the child has never seen before, but not to a puppy that may share many features with the kitten. Similarly, a child having learned the adjective *wug* should be able to understand that *wugness* is an acceptable way to nominalize the adjective.

There is good evidence that children are able to effectively generalize. For example, @xuWordLearningBayesian2007 have demonstrated that children are able to generalize a word's meaning beyond the literal context it is heard in. Specifically, they examined how children come to learn that a novel word refers to a hypernym such as dalmatian as opposed to the respective hyponym (dog). They found that when encountering only one example of the novel word with a dalmatian, children assume that the word refers to the hyponym. However, when they encounter the word in three separate instances, each of which referred to a dalmatian, the children instead assume that the word refers to the hypernym. This, @xuWordLearningBayesian2007 argued, is the rational conclusion because it seems unlikely that the word would mean dog and occur three separate times with a dalmatian (without occurring with any other dogs).

Additionally, @smith1996namingyoungchildren have even demonstrated that children can generalize a word's meaning across several different contexts, even when each individual context doesn't indicate what the meaning of the word is.

While word-learning is an important form of generalization, it is by no means the only type of generalization. A less intuitive form of generalization that is extremely common is semantic extension. Impressively, even after learning a word's meaning, humans will sometimes extend that word to encompass novel contexts. For example, children will sometimes use the word *kitty* to mean all four-legged animals. Further, adults will sometimes accept intransitive verbs in transitive contexts [@theakstonRoleEntrenchmentChildren2004].

It is unclear, however, what factors lead to semantic extension. Mainly, the literature has demonstrated that semantic extension may be driven by accessibility, but it may instead be driven by informativity.

Accessibility, as the term suggests, refers to how easy it is for a learner to access a given representation. Accessibility is driven by many different factors, such as the frequency of the representation or the current attention of the learner. For example, frequent words are thought to be easier to access, evidenced by faster reading times, higher skip-rates in reading, and phonetic reduction in spoken speech. Similarly, while in a random context the representation for *wolf* may not be very accessible, this may not be the case if you are visiting the Wolf Conservation Center in New York, where the representation may be significantly more accessible given the context.

Accessibility provides a fairly intuitive account for semantic extension: when encountering a new context, the most accessible word that can fit the new context seems like a reasonable approach. The role of accessibility within semantic extension has also been posited before by @harmonPuttingOldTools2017. @harmonPuttingOldTools2017 demonstrated the plausibility of this through an artificial language learning experiment. In their experiment, participants were taught a morphological artificial language that consisted of learning the correct words for a creature. In their artificial language, the stem for each creature could be followed by one of four suffixes. In their language, these suffixes varied in how frequent they occurred. Two of these were diminutive singular suffixes and two were big plural suffixes. In the test case, participants were asked to produce the correct suffix to express the diminutive plural meaning. They found that in production, participants were more likely to use the frequent suffix. Interestingly, they also found that in a two-alternative forced choice task (2afc), participants were equally likely to select each suffix. They argued that this was because in a two-alternative forced choice task, both forms were made equally accessible.

On the other hand, informativity – the extent to which a specific linguistic cue is informative about the meaning – also clearly plays a large role in language learning. Indeed, this is the foundation that underlies associative, or error-driven, learning theories. Perhaps one of the most seminal demonstrations of this can be found in @smith2008infantsrapidlylearn where the authors demonstrated that children can learn the meanings of words despite there not being enough information on each individual trial to learn the word's meaning. Instead, learners must track the statistics across different environments to learn the word's meaning, which they did successfully.

Indeed, the results of @harmonPuttingOldTools2017 are also compatible with an informativity account since the frequent suffix also occurred with more unique creatures than the infrequent suffix (i.e., it had high type-frequency). This may have led participants to produce the more frequent suffix in a novel context because they believe that it is the most informative option. While the 2afc results seem to contradict this possibility at-first-glance, participants may accept both possibilities when explicitly ask because they are both technically compatible with the new meaning.[^1]
...
Yet another possibility is that both accessibility and informativity may play a role in semantic extension. This seems plausible since humans employ both Hebbian [@hebb1949firststageperceptionhebb] and error-driven learning mechanisms [@ashbyNeurobiologicalTheoryAutomaticity2007, @kapatsinskiChangingMindsChanging2018]. In other words, humans are sensitive to not only frequency, but also informativity. It is possible that both of these play a crucial role in semantic extension.

Further, there is evidence that both drive linguistic productivity [@jarosz2025typetokenfrequency].

### Present Study

Thus in the present study, we expand upon the findings in @harmonPuttingOldTools2017 by examining the effects of type and token frequency independently on semantic extension.
