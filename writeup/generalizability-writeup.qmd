...
A crucial element of language learning is the ability to use that language beyond the exact contexts in which it is encountered. This is not a trivial task since word-frequencies follow a Zipfian distribution [@zipf1949principleleasteffort], with the most frequent word being twice as frequent as the second most frequent word, which in turn is twice as frequent as the third most frequent word (so on and so forth). This means that the majority of words fall in the tail end of the distribution, with humans having encountered them infrequently. As a result, in order to use language outside of the exact context in which it is encountered, language learning mechanisms must enable us to infer meaning beyond the literal contexts.
...
There is good evidence that children are able to effectively generalize, much of which comes from studies examining linguistic productivity. For example, @xuWordLearningBayesian2007 have demonstrated that children are able to generalize a word's meaning beyond the literal context it is heard in. Specifically, they examined how children come to learn that a novel word refers to a hypernym such as dalmatian as opposed to the respective hyponym (dog). They found that when encountering only one example of the novel word with a dalmatian, children assume that the word refers to the hyponym. However, when they encounter the word in three separate instances, each of which referrring to a dalmatian, the children instead assume that the word refers to the hypernym (dalmatian). This, @xuWordLearningBayesian2007 argued, is the rational conclusion because it is unlikely that the word would mean dog and occur three separate times with a dalmatian (without occurring with any other dogs).

Additionally, @smith2008infantsrapidlylearn have demonstrated that humans can generalize a word's meaning across several different contexts, even when each individual context is ambiguous as to the meaning of the word. In order to test this, they showed infants two pictures at a time paired with a single word. While the word was ambiguous within a single trial (it could refer to either image), it was unambiguous across trials. That is, one of the two images that were presented was always constant across trials (for a given word). They found that despite the ambiguity within trials, during the test phase infants spent a longer time looking at the image that the word consistently occurred with. This demonstrates that humans are able to generalize beyond the specific context by using statistical regularities across different contexts.
...
Beyond word-learning, rational learning also plays a role in the learning of phonetic categories. For example, @olejarczukDistributionalLearningErrordriven2018 examined how humans learn new phonetic categories. They found that learners' perceptual categories are disproportinately affected by rare members of a phonetic category. Their findings suggest that learners actively pay attention not just to co-occurrence rates, but also how informative each member is of the phonetic category and update their beliefs in proportion to how surprising their membership is, providing further evidence for rational learning.
...
[^1]: Indeed, @qiu2023taskvariationscalar and @houghtonTaskdependentConsequencesDisfluency2024 have both found that the tasks that participants are engaged in can impact the linguistic processes that they're engaged in. For example, when participants are asked whether it is true that all the images on a card are cats upon hearing the sentence "some animals are cats", they will answer yes despite the implicature that not all the animals on the card are cats.
