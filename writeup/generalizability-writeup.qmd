...
It is important to note here that unchecked semantic extension would render language useless: if the most frequent words are so general as to encompass any meaning then they become meaningless. Language keeps this in check with entrenchment wherein frequent meanings also become entrenched [@theakstonRoleEntrenchmentChildren2004; @harmonPuttingOldTools2017]. That is, with frequently-encountered forms, learners become more confident that said form is not used to express any other meanings. For example, frequent intransitive verbs are less likely to be accepted in a transitive context than infrequent intransitive verbs [@theakstonRoleEntrenchmentChildren2004]. This finding seems paradoxical because it is unclear how frequency can lead to a learner being more willing to extend a form to novel contexts while simultaneously reinforcing their belief that the frequent form expresses a specific meaning and no other meaning.

In order to address this paradox, @harmonPuttingOldTools2017 examined the effects of frequency on semantic extension and entrenchment. They found that frequency leads to semantic extension in production and entrenchment in comprehension: while a learner becomes more likely to use a frequent form in production, they are less likely to accept a novel meaning with a frequent form in comprehension. However, while this resolves the contradiction of frequency of use leading to both extension and entrenchment, it remains unclear what cognitive mechanisms drive semantic extension.

### Factors Driving Semantic Extension
...
Accessibility, as the term suggests, refers to how easy it is for a learner to access a given representation. Accessibility is driven by many different factors, such as the frequency of the representation or the current attention of the learner. For example, frequent words are thought to be easier to access, evidenced by faster reading times [@rayner2009eyemovementsattention; @drieghe2005eyemovementsword], higher skip-rates in reading [@drieghe2005eyemovementsword], and phonetic reduction in spoken speech [@bybee1999usagebasedphonology]. In addition to lexical characteristics, accessibility can also be mediated by the context which the speaker is in. For example, while you are relaxing in your home the representation for *wolf* may not be very accessible, however if you are visiting a wolf conservation center, the representation for *wolf* may be significantly more accessible given the context (hence the classic Psychology adage that one should study in the same classroom in which they will take the exam).

Accessibility plays a large role in language production and processing which in term shapes the way that language changes over time [e.g., @goldberg2021; @morganAbstractKnowledgeDirect2016]. For example, there is an abundance of evidence that binomial ordering preferences (e.g., *bread and butter* vs *butter and bread*) are driven at least in part by accessibility [@morganAbstractKnowledgeDirect2016; @goldberg2021] and that the changing of these ordering preferences over time (e.g., the reversal of the preference of *dad and mom to mom and dad*) reflects changes in the accessibility of that form. Further evidence of the role of accessibility in language processing comes from Russell Tomlin's seminal *fish film* [@tomlin1995focalattentionvoice]. In his study, he presented participants with a short video of one fish eating another fish, with one of the fishes having an arrow over its image. He found that if the image with an arrow over its head was the fish being eaten, participants would describe the image using the passive form. If the image was instead, the fish doing the action of the eating, participants would instead use the active form. The study demonstrates that accessibility plays a crucial role in the way we produce language.
