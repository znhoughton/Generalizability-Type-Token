...
subject_effects$Stem = factor(subject_effects$Stem, levels = c('Familiar', 'Novel'))
...
model_prod_both_stem_rw = brm(frequency ~ condition*meaning*stem_condition + freq_minus_infreq + (1 + stem_condition*meaning + freq_minus_infreq | participant) + (1 | resp_stem),
...
model_prod_rw_stem_suffix_vary = brm(frequency ~ freq_minus_infreq_stem * freq_minus_infreq_suffix + (1 + freq_minus_infreq_stem * freq_minus_infreq_suffix | participant) + (1 | resp_stem),
...
loo_co_results = data.frame(loo_compare(model_human_rw_preds, model_prod_rw_stem_suffix_vary, model_prod_both_stem_rw_simple, model_prod_both_stem_simple_humans)) %>%
  mutate('term' = c('Stem and Suffix Activations as Separate Predictors', 'Original Predictiors Plus RW Activations', 'Only RW Activations', 'Only Original Predictors'))
...
We treatment coded condition such that the intercept was the 12-3 condition. As such, for each condition, a positive coefficient indicates that participants are more likely to choose a novel meaning (relative to the 12-3 condition/intercept). The model syntax is included below in @eq-comprehensionmodel.
...
In order to test our model predictions, we created three Bayesian models to determine which model predicted the data best. The first model was a simple model that modeled frequency as a function of the Rescorla-Wagner predictions with a random intercept for participant and for stem (@eq-rwmodel1). The second model was analogous, except that we divided the activations into stem and meaning activations. For example, instead of having a single activation for `bal.dim.sg` (which was the association between those cues and the frequent suffix minus the association between those cues and the infrequent suffix) we instead calculated the activations separately for the stem, *bal*, and the meaning, dim.sg (@eq-rwmodel2). Finally, in our third statistical model, we included the original predictors in the human model (stem, condition, and meaning) along with the Rescorla-Wagner predictions (@eq-rwmodel3). In all of our models, frequency is whether the frequent or infrequent suffix was chosen in the human data, meaning is whether the meaning was familiar or novel, stem_condition was whether the stem was familiar or novel, freq_minus_infreq was the difference in predicted associations between the frequent and infrequent suffixes, and resp_stem was the individual stem (e.g., *bal*).

$$
\text{frequency} \sim \text{freq\_minus\_infreq} + (\text{freq\_minus\_infreq} | \text{participant}) + (1|\text{resp\_stem})
$$ {#eq-rwmodel1}
...
$$
\begin{aligned}
\text{frequency} &\sim  \text{condition}*\text{meaning}*\text{stem\_condition} + \text{freq\_minus\_infreq} \\ & + (1 + \text{stem\_condition}*meaning + \text{freq\_minus\_infreq} | \text{participant}) \\ & + (1 | \text{resp\_stem})
\end{aligned}
$$ {#eq-rwmodel3}
...
fixefs_model_rw_preds_stem_suffix_vary = bind_rows(fixefs_model_rw_preds_stem_suffix_vary, .id = "ID") %>%
  mutate(term = rep(c('Intercept', 'Stem Activations', 'Suffix Activations', 'Stem:Suffix Activations'), times = 1))
...
Finally, we present a visualization of the model predictions compared to the human data in @fig-rwvshumanpreds. The results demonstrate the the model predictions are quite similar to the human data for the type frequency condition and the type-token frequency condition, however the model predicts an effect of stem familiarity for the token frequency condition that does not appear in the human data. One possible explanation for this difference is that stem identity is a useful predictor of the meaning in the type frequency condition but not in the token frequency condition. That is, in the type frequency condition, certain stems appeared only with one suffix, however in the token frequency condition all the stems occurred with all of the suffixes. This may lead to learners paying attention to the stem identity in the type frequency condition but not in the token frequency condition.

```{r, echo = F, out.width = '80%', fig.align = 'center', warning = F, message = F}
#| label: fig-rwvshumanpreds
#| fig-cap: "Plot of the RW predictions versus the human results. The y-axis is the probability of producing the frequent meaning. The top facet is original meanings and the bottom axis is novel meanings. The results suggest that the model captures the type frequency and token-frequency conditions well, but predicts an effect of stem familiarity where there is none in the human data for the token frequency condition."
#| fig-height: 6
#| fig-width: 7
...
# data_all_conds_logistic2 = read_csv('../Data/data_all_conds_logistic.csv')
# 
# data_all_conds_logistic2$prod_condition = factor(data_all_conds_logistic2$prod_condition, levels = c('familiar', 'novel'), labels = c('Familiar', 'Novel'))
# data_all_conds_logistic2$meaning = factor(data_all_conds_logistic2$meaning, levels = c('original', 'novel'), labels = c('Original', 'Novel'))
# data_all_conds_logistic2$condition = factor(data_all_conds_logistic2$condition, levels = c(1, 2, 3), labels = c('Type Frequency', 'Token Frequency', 'Type-Token Frequency'))

# 
# colnames(data_all_conds_logistic2) = c('Meaning', 'Stem', 'Condition', 'Estimate')
# 
...
# observed_estimates = fixed_effects_df 
# 
# observed_estimates$Stem = factor(observed_estimates$Stem, levels = c('familiar', 'novel'), labels = c('Familiar', 'Novel'))
# observed_estimates$Meaning = factor(observed_estimates$Meaning, levels = c('original', 'novel'), labels = c('Original', 'Novel'))
# observed_estimates$Condition = factor(observed_estimates$Condition, levels = c('cond1', 'cond2', 'cond3'), labels = c('Type Frequency', 'Token Frequency', 'Type-Token Frequency'))
# 
# observed_estimates = observed_estimates %>%
#   mutate('Type' = 'Human')
# 
# all_data = observed_estimates %>% 
#   full_join(data_all_conds_logistic2)
#all_data$Prob = invlogit(all_data$Estimate)

#write_csv(all_data, '../Data/model_preds_and_human_data.csv')
all_data = read_csv('../Data/model_preds_and_human_data.csv')

observed_vs_model_plot = ggplot(data = all_data, aes(x = Type, y = Prob, fill = Stem)) +
  geom_col(position = "dodge") +
