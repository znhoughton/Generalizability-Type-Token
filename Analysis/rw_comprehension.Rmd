...
data_cond1_rw_matrix = map_dfr(1:n_sims, function(i) {
  result = as.data.frame(Rescorla(Alpha = 0.45, Beta = 0.25, cuesOutcomes = data_cond1[sample(1:nrow(data_cond1)), ], logistic = T))
  result = rownames_to_column(result, var = 'cue')
  result$sim = i
  result
})
...
data_cond2_rw_matrix = map_dfr(1:n_sims, function(i) {
  result = as.data.frame(Rescorla(Alpha = 0.45, Beta = 0.25, cuesOutcomes = data_cond2[sample(1:nrow(data_cond2)), ], logistic = T))
  result = rownames_to_column(result, var = 'cue')
  result$sim = i
  result
})
...
data_cond3_rw_matrix = map_dfr(1:n_sims, function(i) {
  result = as.data.frame(Rescorla(Alpha = 0.45, Beta = 0.25, cuesOutcomes = data_cond3[sample(1:nrow(data_cond3)), ], logistic = T))
  result = rownames_to_column(result, var = 'cue')
  result$sim = i
  result
})
...
data_cond4_rw_matrix = map_dfr(1:n_sims, function(i) {
  result = as.data.frame(Rescorla(Alpha = 0.45, Beta = 0.25, cuesOutcomes = data_cond4[sample(1:nrow(data_cond4)), ], logistic = T))
  result = rownames_to_column(result, var = 'cue')
  result$sim = i
  result
})
...
data_cond5_rw_matrix = map_dfr(1:n_sims, function(i) {
  result = as.data.frame(Rescorla(Alpha = 0.45, Beta = 0.25, cuesOutcomes = data_cond5[sample(1:nrow(data_cond5)), ], logistic = T))
  result = rownames_to_column(result, var = 'cue')
  result$sim = i
  result
})
...
data_cond6_rw_matrix = map_dfr(1:n_sims, function(i) {
  result = as.data.frame(Rescorla(Alpha = 0.45, Beta = 0.25, cuesOutcomes = data_cond6[sample(1:nrow(data_cond6)), ], logistic = T))
  result = rownames_to_column(result, var = 'cue')
  result$sim = i
  result
})
...
priors_m1b = c(
  prior(student_t(3, 0, 0.001), class = 'Intercept'), #0.5 might seem small, but in logistic regression this is still quite large since a-prior exp(1) = 2.7 is still plausible.
  prior(student_t(3, 0, 0.001), class = 'b'),
  prior(student_t(3, 0, 0.001), class = 'sd'))
...
run_comprehension <- function(
  data_comp,
  data_conds,      # list of 6 configural datasets
  human_comp_data, # must contain: stem_condition, condition_numeric, Estimate
  n_sims,
  Alpha,
  Beta
){
  ...
  # -------------------------------------
  # internal helper to simulate RW model
  # -------------------------------------
  run_rw <- function(data) {
    map_dfr(1:n_sims, function(i){
      Rescorla(
        cuesOutcomes = data[sample(nrow(data)), ],
        logistic = TRUE,
        Alpha = Alpha,
        Beta  = Beta
      ) %>%
  ...
  # run RW on all 6 conditions
  rw_results_list <- purrr::map(data_conds, run_rw)

  # -------------------------------------
  # preprocess comprehension data
  # -------------------------------------
  data_comp <- data_comp %>%
    mutate(
      label = if_else(suffix != "none",
                      paste0(stem, "_", suffix),
                      stem),
      row_id = row_number()
    )

  # expand across simulations (same logic as production)
  ...
  process_one_condition <- function(comp_data, rw_data, cond_num){
    ...
    comp_data %>%
      ...
      group_by(row_id, sim) %>%
      summarize(
        big_activations = sum(big, na.rm=TRUE),
        pl_activations  = sum(pl, na.rm=TRUE),
        dim_activations = sum(dim, na.rm=TRUE),
        sg_activations  = sum(sg, na.rm=TRUE),
        .groups="drop"
      ) %>%
  ...
  # run all 6 conditions together
  results <- purrr::map2_dfr(
    seq_along(rw_results_list),
    rw_results_list,
    ~ process_one_condition(data_comp, .y, .x)
  )

  # -------------------------------------
  # recode into 3 comprehension conditions
  # -------------------------------------
  results <- results %>%
    mutate(
      language = case_when(
      ...
      condition_numeric = case_when(
        (condition == 1 & frequency == 1) |
        (condition == 3 & frequency == 1) ~ "12-12",

        (condition == 1 & frequency == 0) |
        (condition == 2 & frequency == 1) ~ "12-3",

        (condition == 2 & frequency == 0) |
        (condition == 3 & frequency == 0) ~ "3-3"
      )
    )

  # -------------------------------------
  # summarize by stem × condition_numeric
  # -------------------------------------
  results <- results %>%
    group_by(stem_condition, condition_numeric) %>%
    summarize(
      mean_pred = mean(new_minus_old),
      ci2.5     = quantile(new_minus_old, 0.025),
      ci97.5    = quantile(new_minus_old, 0.975),
      .groups   = "drop"
    )

  # -------------------------------------
  # join against human data + MSE
  # -------------------------------------
  results <- results %>%
    left_join(
      human_comp_data,
      by = c("stem_condition", "condition_numeric")
    ) %>%
    mutate(
      mse = (mean_pred - Estimate)^2
    )
...
```{r}
## --- extract fixed effects and turn rownames into a column ---
...
human_cells <- expand.grid(
  ...
  mutate(
    ...
    # treatment-coded condition
    cond = case_when(
      condition_numeric == "12-12" ~ b["condition_numeric12M12"],
      condition_numeric == "3-3"   ~ b["condition_numeric3M3"],
      TRUE                         ~ 0   # "12-3" baseline
    ),

    # interaction uses same sum contrast:
    # coefficient * S * 1(condition)
    int = case_when(
...
# test = run_comprehension(
#           data_comp = data_comp,
#           human_comp_data = human_cells,
#           data_conds = list(
#             data_cond1,
#             data_cond2,
#             data_cond3,
...
results_lowest_mse_comp = results_all %>%
  ungroup() %>%
  filter(mean_mse == min(mean_mse))
  
results_lowest_mse_comp$SemAlpha
results_lowest_mse_comp$Beta
results_lowest_mse_comp$Alpha
```

```{r}
write_csv(results_all, 'Data/results_all_comp.csv')
```
