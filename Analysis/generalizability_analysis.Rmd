...
data_prod = data_prod %>%
  mutate('meaning' = case_when(picture_meaning != 'dim_pl' ~ 'original',
                               .default = 'novel'
  )) %>%
  mutate(frequent = case_when(
    language == 'dan' & picture_meaning == 'big_pl' ~ 1,
    language == 'nem' & picture_meaning == 'dim_sg' ~ 1,
    picture_meaning == 'dim_pl' ~ 1,
    .default = 0)) %>%
  mutate(infrequent = case_when(
    language == 'nem' & picture_meaning == 'big_pl' ~ 1,
    language == 'dan' & picture_meaning == 'dim_sg' ~ 1,
    picture_meaning == 'dim_pl' ~ 1,
    .default = 0)) %>%
  rowwise() %>%
  mutate(frequency = case_when(
    frequent == 1 & infrequent == 0 ~ 'frequent',
    infrequent == 1 & frequent == 0 ~ 'infrequent',
    frequent == 1 & infrequent == 1 ~ NA_character_,  # We will handle this case later
    TRUE ~ NA_character_
  )) %>%
  ungroup() 
...
data_prod = data_prod %>%
  ...
  bind_rows(
    data_prod %>%
      filter(frequent == 1 & infrequent == 1) %>%
      mutate(frequency = 'frequent'),
    data_prod %>%
      filter(frequent == 1 & infrequent == 1) %>%
      mutate(frequency = 'infrequent')
  ) %>%
  mutate('freq_choice_dv' = case_when(
    freq_choice == 'freq' & frequency == 'frequent' ~ 1,
    freq_choice == 'infreq' & frequency == 'infrequent' ~ 1, 
    #freq_choice %in% c('freq', 'infreq') ~ 1,
                       .default = 0)
  )

data_prod$participant = factor(data_prod$participant)
data_prod$participant = factor(data_prod$condition)
...
original_meaning_freq_prod_data = data_analysis %>%
  filter(task %in% c('prod')) %>%
  filter((language == 'dan' & picture_meaning == 'big_pl') | (language == 'nem' & picture_meaning == 'dim_sg')) %>%
  ungroup() %>%
  mutate(freq_choice_dv = case_when(
    freq_choice == 'freq' ~ 1,
    .default = 0
  )) %>%
  mutate(meaning = 'original', frequency = 'frequent')
    
novel_meaning_freq_prod_data = data_analysis %>%
  filter(task %in% c('prod')) %>%
  filter(picture_meaning %in% c('dim_pl')) %>%
  ungroup() %>%
  mutate(freq_choice_dv = ifelse(freq_choice == 'freq', 1, 0)) %>%
  mutate(meaning = 'novel', frequency = 'frequent')

original_meaning_infreq_prod_data = data_analysis %>%
  filter(task %in% c('prod')) %>%
  filter((language == 'nem' & picture_meaning == 'big_pl') | (language == 'dan' & picture_meaning == 'dim_sg')) %>%
  ungroup() %>%
  mutate(freq_choice_dv = case_when(
    freq_choice == 'infreq' ~ 1,
    .default = 0
  )) %>%
  mutate(meaning = 'original', frequency = 'infrequent')

novel_meaning_infreq_prod_data = data_analysis %>%
  filter(task %in% c('prod')) %>%
  filter(picture_meaning %in% c('dim_pl') ) %>%
  ungroup() %>%
  mutate(freq_choice_dv = ifelse(freq_choice == 'infreq', 1, 0)) %>%
  mutate(meaning = 'novel', frequency = 'infrequent')
  
  
data_test_all = original_meaning_freq_prod_data %>%
  full_join(novel_meaning_freq_prod_data) %>%
  full_join(original_meaning_infreq_prod_data) %>%
  full_join(novel_meaning_infreq_prod_data)
...
data_2afc_cond1 = data_2afc %>% filter(condition == 1)
data_2afc_cond2 = data_2afc %>% filter(condition == 2)
data_2afc_cond3 = data_2afc %>% filter(condition == 3)

data_analysis_comprehension_cond1 = data_analysis_comprehension %>% filter(condition == 1)
data_analysis_comprehension_cond2 = data_analysis_comprehension %>% filter(condition == 2)
data_analysis_comprehension_cond3 = data_analysis_comprehension %>% filter(condition == 3)



# 
# data_prod_test = data_prod %>%
#   #filter(!is.na(frequency)) %>%
#   group_by(condition, meaning, frequency) %>%
#   summarize(Prob = mean(freq_choice_dv, sd = sd(freq_choice_dv)))
# 
# 
# data_prod_test2 = data_test_all %>%
#   #filter(!is.na(frequency)) %>%
#   group_by(condition, meaning, frequency) %>%
#   summarize(Prob = mean(freq_choice_dv, sd = sd(freq_choice_dv)))

# ggplot(data_prod_test2, aes(x = meaning, y = Prob, fill = frequency)) +
...
full_model_production_for_paper = brm(freq_choice_dv ~ frequency*meaning*condition + (frequency*meaning | participant) + (frequency*meaning*condition | resp_stem),
                                       data = data_prod,
                                       family = bernoulli(link = 'logit'),
                                       iter = 8000, 
                                       warmup = 4000,
                                       ...
                                       prior = priors_m1,
                                       #control = list(adapt_delta = 0.99),
                                       file = '../Data/full_model_production_for_paper')

cond1_model_production_for_paper = brm(freq_choice_dv ~ frequency*meaning + (frequency*meaning | participant) + (frequency*meaning | resp_stem),
                                       data = data_prod_cond1,
                                       family = bernoulli(link = 'logit'),
                                       iter = 6000, 
                                       warmup = 3000,
                                       ...
                                       prior = priors_m1,
                                       #control = list(adapt_delta = 0.99),
                                       file = '../Data/cond1_model_production_for_paper')

cond2_model_production_for_paper = brm(freq_choice_dv ~ frequency*meaning + (frequency*meaning | participant) + (frequency*meaning | resp_stem),
                                       data = data_prod_cond2,
                                       family = bernoulli(link = 'logit'),
                                       iter = 6000, 
                                       warmup = 3000,
                                       ...
                                       prior = priors_m1,
                                       #control = list(adapt_delta = 0.99),
                                       file = '../Data/cond2_model_production_for_paper')

cond3_model_production_for_paper = brm(freq_choice_dv ~ frequency*meaning + (frequency*meaning | participant) + (frequency*meaning | resp_stem),
                                       data = data_prod_cond3,
                                       family = bernoulli(link = 'logit'),
                                       iter = 6000, 
                                       warmup = 3000,
                                       ...
                                       prior = priors_m1,
                                       #control = list(adapt_delta = 0.99),
                                       file = '../Data/cond3_model_production_for_paper')
